...
#'Zfull': ('fonts/Zfull-GB.ttf', 16, 16),
    ...
    try:
        while True:
            lcd.displayImage(disp.txtDisp(demo1, 'Wenq', 30, resize=1, align=1))
            lcd.delay(1000)
            lcd.clearLCD()
            lcd.displayImage(disp.txtDisp(en_multi, 'Comfortaa', resize=1))
            lcd.delay(2000)
            lcd.clearLCD()
            lcd.displayImage(disp.txtDisp(en_multi, 'PoiretOne', resize=1))
            lcd.delay(2000)
            lcd.clearLCD()
            lcd.displayImage(disp.txtDisp(en_multi, 'Rokkitt', resize=1))
            lcd.delay(2000)
            lcd.clearLCD()
            lcd.displayImage(disp.txtDisp(en_multi, 'Wenq', resize=1))
            lcd.delay(2000)
            lcd.clearLCD()
            lcd.displayImage(disp.txtDisp(en_multi, 'NotoSansSC', resize=1))
            lcd.delay(2000)
            lcd.clearLCD()
            lcd.displayImage(disp.txtDisp(en_multi, 'NotoSerifSC', resize=1))
            lcd.delay(2000)
            lcd.clearLCD()
            ...
            lcd.delay(2000)
            lcd.clearLCD()
            lcd.displayImage(disp.txtDisp(demo3, 'Wenq', 30, resize=1, align=1))
            lcd.delay(1000)
            lcd.clearLCD()
            ...
            lcd.delay(2000)
            lcd.clearLCD()
            lcd.displayImage(disp.txtDisp(demo4, 'Wenq', 30, resize=1, align=1))
            lcd.delay(1000)
            lcd.clearLCD()
            lcd.displayImage(disp.txtDisp(excited, 'NotoSerifSC', 99, resize=1))
            lcd.delay(2000)
            lcd.clearLCD()
            lcd.displayImage(disp.txtDisp(demo5, 'Wenq', 30, resize=1, align=1))
            lcd.delay(1000)
            lcd.clearLCD()
